...
class action extends app
{
	...
	public function scoress(){
        ...
        $name=$this->ev->get('name');
        $phonenumber=$this->ev->get('phonenumber');
        $payway=$this->ev->get('payway');
        $money=$this->ev->get('money');
        $moneyleft=$this->ev->get('moneyleft');
        $score=$this->ev->get('score');
        $scoreopenid=$_SESSION['ifscore'];
        $scorenicknam=$_SESSION['ifnickname'];
...
$iftime=date('Y-m-d H-i-s',time());
        $moneyleft=$moneyleft-$money;
		$insert="INSERT INTO `x2_money` (`name`, `payway`, `phonenumber`, `moneyleft`,`money`,`score`,`openid`,`time`,`nickname`) VALUES ('$name', '$payway', '$phonenumber','$moneyleft','$money','$score','$scoreopenid','$iftime','$scorenicknam')";
		$charu=$mysql->query($insert);
if ($charu)
{
		echo "<div style='width:100%;margin: 30% auto;display: block;text-align:center;font:25px red bold;color: red;'>谢谢参与！我们将在6月10日以后统一发放红包！</div>";
}
else{
			echo "<div>提交失败</div>";
		}
	...
	private function makescore()//计算分数
	{
		$questype = $this->basic->getQuestypeList();
		$sessionvars = $this->exam->getExamSessionBySessionid();
...
//其实读写都可以只加入
            $mysql->query("set names 'utf8'");
            $table=$pre.'money';
            $sel="SELECT moneyleft FROM $table WHERE 1 ORDER BY id DESC LIMIT 1";
            ...
            while($arr=$re->fetch_array(MYSQLI_ASSOC))
            {
             ...
             $moneyleft= $arrs[0]['moneyleft'];
            $this->tpl->assign('a',$moneyleft);
	...
	private function selectquestions()
	{
		...
		else
		{
			if($r['examtype'] == 1)
			{
				...
				$message = array(
					'statusCode' => 200,
					"message" => "你已进入答题系统，中途请勿退出！60分以上将获得1-10元随机红包！",
				    "callbackType" => 'forward',
